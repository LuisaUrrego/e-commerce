...
const getFavoritos = async (url) => {
 try {
     const { data }= await axios.get(url + "favoritos")
    // console.log(data)
     return data;
...
const containerCardProducts = document.querySelector(".pintar_favoritos")

//Pintar los productos dentro del contenedor
const printProductsFavoritos = (container, products) => {
...
document.addEventListener ('DOMContentLoaded',  async () => {
   products = await getFavoritos (URL_API)
   printProductsFavoritos(containerCardProducts, products )
